...
export function BlogSection() {
  ...
  // Limit to 6 posts total for display on homepage
  const displayedFeaturedPosts = useMemo(() => {
    return featuredPosts.slice(0, 6);
  }, [featuredPosts]);

  const displayedRegularPosts = useMemo(() => {
    const remainingSlots = 6 - displayedFeaturedPosts.length;
    return regularPosts.slice(0, Math.max(0, remainingSlots));
  }, [regularPosts, displayedFeaturedPosts.length]);
